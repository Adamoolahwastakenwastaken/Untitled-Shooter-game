...
def collide_with_walls(sprite, group, dir):
    if dir == 'x':
        hits = pg.sprite.spritecollide(sprite, group, False, collide_with_hit_rect)
        if hits:
            if sprite.vel.x > 0:
    ...
    def get_keys(self):
        ...
        if keys[pg.K_SPACE]:
            ...
            if now - self.last_shot > BULLET_RATE:
                ...
                self.vel = vec(-KICK_BACK).rotate(-self.rot)
...
class MuzzleFlash(pg.sprite.Sprite):
    def __init__(self, game, pos):
        ...
        self.game = game
        size = randint(20, 50)
        self.image = pg.transform.scale(choice(game.Muzzle_images), (size, size))
        self.rect = self.image.get_rect()
        self.pos = pos
        self.rect.center = pos
        self.spawn_time = pg.time.get_ticks()
